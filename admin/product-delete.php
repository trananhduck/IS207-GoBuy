...
$querry = $pdo->prepare("DELETE FROM table_product WHERE p_id=?");
$querry->execute(array($_REQUEST['id']));

// Xóa ảnh sản phẩm khỏi bảng table_product_photo
$querry = $pdo->prepare("DELETE FROM table_product_photo WHERE p_id=?");
$querry->execute(array($_REQUEST['id']));
...
$querry = $pdo->prepare("DELETE FROM table_rating WHERE p_id=?");
$querry->execute(array($_REQUEST['id']));

// Xóa thông tin thanh toán liên quan đến sản phẩm khỏi bảng table_payment
$querry = $pdo->prepare("SELECT * FROM table_order WHERE product_id=?");
$querry->execute(array($_REQUEST['id']));
$result = $querry->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $querry1 = $pdo->prepare("DELETE FROM table_payment WHERE payment_id=?");
    $querry1->execute(array($row['payment_id']));
}

// Xóa đơn hàng liên quan đến sản phẩm khỏi bảng table_order
$querry = $pdo->prepare("DELETE FROM table_order WHERE product_id=?");
$querry->execute(array($_REQUEST['id']));

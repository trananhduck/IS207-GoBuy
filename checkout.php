                ...
                <?php else: ?>
                    ...
                    <div class="cart">
                        <table class="table table-responsive table-hover table-bordered">
                            ...
                            <?php
                            // Sử dụng cust_s_province thay vì cust_province
                            $province_id = (int)$_SESSION['customer']['cust_s_province'];
                            // Xóa dòng echo gỡ lỗi
                            // echo "Province ID: " . $_SESSION['customer']['cust_province'];

                            $query = $pdo->prepare("SELECT * FROM table_shipping_cost WHERE province_id = ?");
                            $query->execute(array($province_id));
                            $result = $query->fetch(PDO::FETCH_ASSOC);

                            if ($result) {
                                $shipping_cost = $result['amount'];
                            } else {
                                $shipping_cost = 10; // Giá trị mặc định nếu không có dữ liệu
                                // Ghi log để kiểm tra lỗi
                                error_log("Không tìm thấy chi phí vận chuyển cho tỉnh ID: " . $province_id);
                            }
                            ?>
                            <tr>
                    ...
                    <div class="billing-address">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <a href="customer-billing-shipping-update.php"><button
                                        class="btn btn-primary"><?php echo 'Cập nhật thông tin giao hàng' ?></button></a>
                            </div>
                        </div>
                    </div>

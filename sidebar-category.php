        ...
        if ($current_top_category > 0) {
            ...
            if ($tcat_name) {
                ...
                foreach ($mcats as $mcat) {
                    ...
                    echo '<span data-toggle="collapse" data-parent="#menu-group-1" href="#cat-lvl2-id-special-' . $j . '" class="sign"><i class="fa fa-caret-right fa-2x"></i></span>';
        ...
        <li class="cat-level-1 deeper parent <?php echo $active_class; ?>">
            <a class="<?php echo $active_class; ?>"
                ...
                <span data-toggle="collapse" data-parent="#menu-group-1" href="#cat-lvl1-id-<?php echo $i; ?>"
                    class="sign"><i class="fa fa-caret-right fa-2x"></i></span>
                    ...
                    <a class="<?php echo $active_class_mid; ?>"
                        ...
                        <span data-toggle="collapse" data-parent="#menu-group-1"
                            href="#cat-lvl2-id-<?php echo $i . $j; ?>" class="sign"><i
                                class="fa fa-caret-right fa-2x"></i></span>
...
@media only screen and (max-width: 991px) {
    ...
    @media only screen and (max-width: 767px) {
        #left {
            max-width: 90%;
            /* Giảm xuống 90% khi màn hình nhỏ hơn 800px */
            width: 90%;
        }
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    let menuContainer = document.getElementById("cat-lvl1-current");
    if (!menuContainer) return; // Nếu không tồn tại, thoát sớm

    let menuLinks = menuContainer.querySelectorAll('a');

    menuLinks.forEach(function(link) {
        link.addEventListener("click", function(e) {
            let parentLi = this.closest("li");
            let submenu = parentLi.querySelector("ul.children");
            let icon = this.querySelector(".sign i");

            if (submenu) {
                e.preventDefault(); // Ngăn chặn chuyển trang ngay

                let isOpen = submenu.style.display === "block";

                // Đóng tất cả danh mục con trong cat-lvl1-current
                menuContainer.querySelectorAll("ul.children").forEach(function(ul) {
                    ul.style.display = "none";
                });
                menuContainer.querySelectorAll(".sign i").forEach(function(iTag) {
                    iTag.classList.remove("fa-caret-down");
                    iTag.classList.add("fa-caret-right");
                });

                // Nếu chưa mở, thì mở danh mục này
                if (!isOpen) {
                ...
                // Nếu không có danh mục con, vẫn cho phép chuyển trang
...
// document.addEventListener("DOMContentLoaded", function () {
//     let menuContainer = document.getElementById("cat-lvl1-current"); 
//     if (!menuContainer) return; // Nếu không có danh mục, thoát
...
//             // Nếu có danh mục con, mở rộng menu và chuyển trang
//             if (submenu) {
...
//                 // Không ngăn chặn chuyển hướng => vẫn mở trang mới
//             }
//         });
//     });
// });
